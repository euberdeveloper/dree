...
function skip(child, options, depth) {
    return (!options.symbolicLinks && child.isSymbolicLink)
        ...
        || (Array.isArray(options.exclude) && options.exclude.find(function (pattern) { return pattern.test(child.path); }) !== undefined)
...
function _parse(children, prefix, options, depth) {
    ...
    children.forEach(function (child, index) {
        if (!skip(child, options, depth)) {
            ...
            var line = (index === children.length - 1 || index === children.length - 2 && skip(children[index + 1], options, depth)) ? '└' + last : '├' + last;
 ...
 */
function parse(dirTree, options) {
    var result = '';
