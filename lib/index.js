...
function _dree(root, path, depth, options, onFile, onDir) {
    ...
    switch (type) {
        case Type.DIRECTORY:
            ...
            fs_1.readdirSync(path).forEach(file => {
                const child = _dree(root, path_1.resolve(path, file), depth + 1, options, onFile, onDir);
                if (child != null) {
                    children.push(child);
                }
            });
 ...
 */
function dree(path, options, onFile, onDir) {
    const root = path_1.resolve(path);
    return _dree(root, root, 0, mergeOptions(options), onFile, onDir);
}
exports.dree = dree;
